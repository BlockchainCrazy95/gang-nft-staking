...
use mpl_token_metadata::{state::Metadata, state::TokenMetadataAccount, ID as MetadataProgramID};
...
pub fn handler(ctx: Context<StakeNft>) -> Result<()> {
    ...
    let staked_amount = accts.user_state.staked_amount;
    accts.user_state.last_claim_time[staked_amount as usize] = current_time as u64;
    accts.user_state.potion_nfts[staked_amount as usize] = accts.nft_mint.key();
    accts.user_state.staked_amount = staked_amount + 1;
